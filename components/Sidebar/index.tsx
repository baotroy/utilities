  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        ...
        <button ref={trigger} onClick={() => setSidebarOpen(!sidebarOpen)} aria-controls="sidebar" aria-expanded={sidebarOpen} className="hidden">
                ...
                <ul className="mb-6 flex flex-col gap-1.5">
                  {group.items.map((item, index) => {
                    if (item.children?.length) {
                      return (
                        <SidebarLinkGroup key={index} activeCondition={pathname === item.href || pathname.includes(item.slug)}>
                          {(handleClick, open) => {
                            return (
                              <React.Fragment>
                                <LinkItem
                                  ...
                                  href="#"
                                  icon={item.icon}
                                  isFocus={pathname === item.href || pathname.includes(item.slug)}
                                />
                                <ChildrenLinkItems items={item.children} currentPathname={pathname} isOpen={open} />
                              </React.Fragment>
                            );
                          }}
                        </SidebarLinkGroup>
                      ...
                      return <LinkItem key={index} label={item.label} href={item.href} icon={item.icon} isFocus={pathname.includes(item.slug)} />;
                    }
                  })}
                </ul>
              </div>
